...
class DetailMemoryFragment : Fragment(R.layout.fragment_detail_memory), MenuProvider {
    ...
    private fun updateUI() {
        ...
        if (currentMemory.memoryImagePath != null) {
            val photoFile = File(currentMemory.memoryImagePath!!)
            if (photoFile.exists()) {
                ...
                binding.memoryImage.setImageURI(photoUri)
            } else {
                Toast.makeText(requireContext(), "Photo file not found", Toast.LENGTH_SHORT).show()
            }
        } else {
            binding.memoryImage.apply {
                setImageResource(R.drawable.default_memory_detail_image)
